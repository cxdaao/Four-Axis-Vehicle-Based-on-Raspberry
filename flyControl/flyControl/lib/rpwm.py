...
class data():
        ...
        def start(self, _233c):
            threads = []
            pi = pigpio.pi()
            t1 = threading.Thread(target=pi.callback, args=(12, pigpio.EITHER_EDGE, self.mycallback1))
            t2 = threading.Thread(target=pi.callback, args=(16, pigpio.EITHER_EDGE, self.mycallback2))
            t3 = threading.Thread(target=pi.callback, args=(20, pigpio.EITHER_EDGE, self.mycallback3))
            t4 = threading.Thread(target=pi.callback, args=(21, pigpio.EITHER_EDGE, self.mycallback4))
            t5 = threading.Thread(target=pi.callback, args=(26, pigpio.EITHER_EDGE, self.mycallback5))
            t6 = threading.Thread(target=pi.callback, args=(19, pigpio.EITHER_EDGE, self.mycallback6))
            threads.append(t1)
            threads.append(t2)
            threads.append(t3)
            threads.append(t4)
            ...
            for t in threads:
                ...
                t.start()
            while True:
                _233c['t1h'] = self.get_t1h()
                _233c['t2h'] = self.get_t2h()
                _233c['t3h'] = self.get_t3h()
                _233c['t4h'] = self.get_t4h()
                _233c['t5h'] = self.get_t5h()
                _233c['t6h'] = self.get_t6h()
                time.sleep(0.1)
